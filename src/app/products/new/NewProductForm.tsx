...
export default function NewProductForm() {
  ...
  const [form, setForm] = useState({
    ...
    image_url: "",
    seller_name: "",
    category: "기타" as ProductCategory,
  });
  ...
  function handleChange(e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) {
    ...
    setForm((prev) => {
      ...
      if (name === "title" && !categoryTouched) {
        next.category = value.trim() ? detectCategory(value) : "기타";
      }
      return next;
    });
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm p-6 flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        {!categoryTouched && form.title.trim() && (
          <p className="text-xs text-teal-500">제목 기반으로 자동 선택되었습니다.</p>
        )}
      </div>
